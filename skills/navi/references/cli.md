...
writes each bar's `plot()` values and any alerts to stdout as NDJSON. It is the first
...
over these bars and show me the values", and for an alert-only check with `--bars none`:
...
```bash
jq -c '{type:"bar",data:.}' bars.json | navi run script.nv --bars all
```
...
| Flag | Effect |
| --- | --- |
| `--bars <none\|realtime\|all>` | Which bar lines to write. Defaults to `realtime`, so **pass `--bars all` to see a historical replay**. `none` leaves an alert-only monitor. |
