...
| 名称 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `points` | <code>series&nbsp;<a class="stdlib-ref" href="/zh-CN/api/stdlib/prelude/Array">Array&lt;<a class="stdlib-ref" href="/zh-CN/api/stdlib/chart/Point">chart.Point</a>&gt;</a></code> |  | `chart.point` 值的数组，按照连接顺序定义折线的顶点。 |
...
| `xloc` | <code>series&nbsp;string</code> | `xloc.bar_index` | 确定如何解释每个 `chart.point` 的 x 坐标。使用 `xloc.bar_index` 将 x 视为 bar 索引，或使用 `xloc.bar_time` 将 x 视为 UNIX 时间戳。选修的。默认为 `xloc.bar_index`。 |
| `line_color` | <code>series&nbsp;color</code> | `color.BLUE` | 线段的颜色。选修的。默认为 `color.BLUE`。 |
| `fill_color` | <code>series&nbsp;color</code> | `na` | 当多段线闭合时用于填充多段线内部的颜色。 `na` 表示不填充。选修的。默认为 `na`。 |
| `line_style` | <code>series&nbsp;string</code> | `line.style_solid` | 线条的风格。可能的值：`LineStyle.Solid`、`LineStyle.Dotted`、`LineStyle.Dashed`、`LineStyle.ArrowLeft`、`LineStyle.ArrowRight`、`LineStyle.ArrowBoth`。选修的。默认为 `LineStyle.Solid`。 |
...
**类型:** <code>series&nbsp;<a class="stdlib-ref" href="/zh-CN/api/stdlib/prelude/Array">Array&lt;<a class="stdlib-ref" href="/zh-CN/api/stdlib/prelude/Polyline">Polyline</a>&gt;</a></code>
