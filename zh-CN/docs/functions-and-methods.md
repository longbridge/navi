...
多数情况下你不用写类型参数——Navi 会自动推断。

**从参数推断。** 当类型参数出现在某个参数类型里时，从传入的实参推断：

```navi
fn first<T>(arr: Array<T>): T {
    arr.get(0);
}
let x = first([1, 2, 3]);   // T 推断为 int
```

**从上下文推断。** 当类型参数只出现在*返回*类型里时，没有实参可供推断，Navi 会从上下文推断——变量声明的类型，或实参位置的期望类型：
...
let a: float = make();      // 从标注推断 T 为 float
let b: int = make();        // T 推断为 int
let c = scale(make());      // T 推断为 float（scale 的参数是 float）
```

**显式指定。** 当没有上下文可推断时，在函数名后用 `<...>` 给出类型参数（前面没有 `::`）：

```navi
let n = make<int>();        // 显式给出 T
```

只出现在返回类型、又没有上下文、也没有显式类型参数的泛型参数会报错：

```navi
let a = make();             // 错误：无法推断泛型类型 `T`
```
...
- [自定义类型与枚举](/zh-CN/docs/custom-types) — 用户定义类型、枚举和新类型声明
