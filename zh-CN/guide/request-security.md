...
`lookahead.on` 使表达式在周期内的第一个图表 bar 就能看到高时间框架 bar 的**最终值**，而不是正在形成中的值。这会在历史 bar 中引入未来数据泄漏——仅在明确需要时使用。

<img class="light-only" src="/request-security-lookahead.svg" alt="lookahead.off 与 lookahead.on 对比" style="width:100%;max-width:600px;">
<img class="dark-only" src="/request-security-lookahead-dark.svg" alt="lookahead.off 与 lookahead.on 对比" style="width:100%;max-width:600px;">
...
`var` 和 `varip` 变量需在脚本顶层声明，不能在 `expression` 参数内部声明。
子实例会在请求的标的/时间框架上执行完整程序体，因此顶层的 `var` 状态会在**请求的**时间框架的各个 bar 之间独立累积：
...
- [数据提供者](/zh-CN/guide/integration/vm-data-providers) — 为 `request.security()` 提供 K 线数据
- [安全与执行限制](/zh-CN/guide/integration/vm-security) — 嵌套深度和调用次数限制
