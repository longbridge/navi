...
if (-not $Version) {
    $Metadata = Invoke-RestMethod "$CdnBase/latest.json"
...
try {
    Invoke-WebRequest "$CdnBase/$Asset" -OutFile "$Temp\$Asset"
    $Expected = ((Invoke-RestMethod "$CdnBase/$Asset.sha256") -split '\s+')[0]
