...
多數情況下你不用寫型別參數——Navi 會自動推斷。

**從參數推斷。** 當型別參數出現在某個參數型別裡時，從傳入的實參推斷：

```navi
fn first<T>(arr: Array<T>): T {
    arr.get(0);
}
let x = first([1, 2, 3]);   // T 推斷為 int
```

**從上下文推斷。** 當型別參數只出現在*回傳*型別裡時，沒有實參可供推斷，Navi 會從上下文推斷——變數宣告的型別，或實參位置的預期型別：
...
let a: float = make();      // 從標註推斷 T 為 float
let b: int = make();        // T 推斷為 int
let c = scale(make());      // T 推斷為 float（scale 的參數是 float）
```

**顯式指定。** 當沒有上下文可推斷時，在函式名後用 `<...>` 給出型別參數（前面沒有 `::`）：

```navi
let n = make<int>();        // 顯式給出 T
```

只出現在回傳型別、又沒有上下文、也沒有顯式型別參數的泛型參數會報錯：

```navi
let a = make();             // 錯誤：無法推斷泛型型別 `T`
```
...
- [自訂類型與枚舉](/zh-HK/docs/custom-types) — 使用者定義類型、枚舉和新型別宣告
