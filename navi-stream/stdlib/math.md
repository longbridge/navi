...
| mod(a: `number`, b: `number`): `number`                 | Returns the remainder of `a` divided by `b`                                                                      |
| pow(a: `number`, b: `number`): `number`                 | Returns `a` raised to the power of `b`                                                                           |
| reverse(n: `number`): `number`                          | Changes the sign of `n`                                                                                          |
| sgn(n: `number`): `number`, sign(n: `number`): `number` | Returns the sign of `n`. If `n > 0`, returns `1`; if `n = 0`, returns `0`; if `n < 0`, returns `-1`              |
| isnil(n: `any?`): `bool`                                | Returns `true` if `n` is `nil`; otherwise, returns `false`                                                       |
