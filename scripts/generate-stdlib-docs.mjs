...
function typeRefToHtml(tr, moduleName) {
  ...
  // Include generic args inside the link so the whole "Map<K, V>" is clickable.
  const nameHtml = url
    ? `<a class="stdlib-ref" href="${url}">${escapeHtml(displayName)}${genericSuffix}</a>`
...
for (const [locale, config] of Object.entries(LOCALE_CONFIGS)) {
  ...
  for (const f of prelude.functions) {
    ...
    const isTypeAssociated =
      ...
      f.overloads.some((ov) =>
        ["method", "staticmethod", "staticproperty"].includes(ov.kind),
      );
