...
Unlike <a class="stdlib-ref" data-key="math::atan" href="/zh-HK/api/math/#atan">atan</a>, this function takes two arguments and determines the correct quadrant using the signs of both. Useful for computing angles between vectors or points.
...
傳回 e（歐拉數）的 `n` 次方。 <a class="stdlib-ref" data-key="math::log" href="/zh-HK/api/math/#log">log</a> 的逆。對於指數增長/衰減計算很有用。
...
傳回 `n` 的自然對數（以 e 為底）。 <a class="stdlib-ref" data-key="math::exp" href="/zh-HK/api/math/#exp">exp</a> 的逆。對於非正值，傳回 `na`。
...
Unlike <a class="stdlib-ref" data-key="math::floor" href="/zh-HK/api/math/#floor">floor</a>, truncation toward zero is symmetric: `trunc(-1.9) = -1`, whereas `floor(-1.9) = -2`.
