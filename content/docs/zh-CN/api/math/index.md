...
Unlike <a class="stdlib-ref" data-key="math::atan" href="/zh-CN/api/math/#atan">atan</a>, this function takes two arguments and determines the correct quadrant using the signs of both. Useful for computing angles between vectors or points.
...
返回 e（欧拉数）的 `n` 次方。 <a class="stdlib-ref" data-key="math::log" href="/zh-CN/api/math/#log">log</a> 的逆。对于指数增长/衰减计算很有用。
...
返回 `n` 的自然对数（以 e 为底）。 <a class="stdlib-ref" data-key="math::exp" href="/zh-CN/api/math/#exp">exp</a> 的逆。对于非正值，返回 `na`。
...
Unlike <a class="stdlib-ref" data-key="math::floor" href="/zh-CN/api/math/#floor">floor</a>, truncation toward zero is symmetric: `trunc(-1.9) = -1`, whereas `floor(-1.9) = -2`.
